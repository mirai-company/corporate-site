...
type NewsItem = {
  ...
  title: string;
  titleEn?: string;
  category: "event" | "media" | "announcement" | "release";
  excerpt: string;
  excerptEn?: string;
  content?: string;
  contentEn?: string;
...
export default function NewsDetailClient({ newsItem, otherNews }: Props) {
  ...
  const imageSrc = newsItem.image ? urlFor(newsItem.image).width(1200).height(675).url() : null;
    ...
    <>
        ...
        <div className="container-custom">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl">
            <nav className="flex items-center gap-2 text-sm mb-8 font-gothic">
              <Link href="/" className="text-[#555555] hover:text-[#0B3D91] transition-colors duration-200 cursor-pointer">{locale === "en" ? "Home" : "ホーム"}</Link>
              <span className="text-[#555555]">/</span>
              <Link href="/news" className="text-[#555555] hover:text-[#0B3D91] transition-colors duration-200 cursor-pointer">{locale === "en" ? "News" : "お知らせ"}</Link>
            </nav>
            <div className="flex items-center gap-4 mb-6">
              <time className="text-sm text-[#0B3D91] font-gothic">{newsItem.date}</time>
              <span className="text-xs bg-[#0B3D91]/10 text-[#0B3D91] px-2 py-1 font-gothic">{categoryLabel}</span>
            </div>
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-heading text-[#333333] leading-relaxed">{title}</h1>
          </motion.div>
        </div>
      </section>
      ...
      {imageSrc && (
        <section className="pb-12">
          <div className="container-custom">
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="max-w-3xl relative aspect-[16/9] bg-gray-100 overflow-hidden">
        ...
        <div className="container-custom">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="max-w-3xl">
            ...
            {newsItem.externalLink && (
              <div className="mt-8">
                <a href={newsItem.externalLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-[#0B3D91] text-white text-sm font-gothic hover:bg-[#072B66] transition-colors duration-200">
            ...
            <div className="mt-12 pt-8 border-t border-gray-200">
              <Link href="/news" className="group inline-flex items-center text-[#0B3D91] text-sm hover:text-[#072B66] transition-colors duration-200 font-gothic cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {otherNews.map((item) => (
                <Link key={item._id} href={\`/news/\${item.id}\`} className="group block bg-white p-6 cursor-pointer transition-shadow duration-200 hover:shadow-md">
                  ...
                  <h3 className="text-sm font-heading text-[#333333] group-hover:text-[#0B3D91] transition-colors duration-200 line-clamp-2">{locale === "en" && item.titleEn ? item.titleEn : item.title}</h3>
                </Link>
              ))}
            </div>
          </div>
